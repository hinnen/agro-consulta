(function () {
    ...
    var dom = {
        ...
        precoYT1: document.getElementById('promo-preco-y-t1'),
        precoYT2: document.getElementById('promo-preco-y-t2'),
        wrapPrecoYT1: document.getElementById('wrap-preco-y-t1'),
        wrapPrecoYT2: document.getElementById('wrap-preco-y-t2'),
        regraVsTabela: document.getElementById('promo-regra-vs-tabela'),
    ...
    function syncTipoUi() {
        ...
        if (dom.wrapPrecoYT1) dom.wrapPrecoYT1.style.display = valorDireto ? 'none' : '';
        if (dom.wrapPrecoYT2) dom.wrapPrecoYT2.style.display = valorDireto ? 'none' : '';
        if (dom.labelQtdX) {
            dom.labelQtdX.textContent = t === 'acima_unidades' ? 'Acima de X unidades' : 'Leve X unidades';
        }
        if (dom.colPrecoPromo) {
            dom.colPrecoPromo.textContent = valorDireto ? 'Padrão / T1 / T2' : 'Regra vs tabela';
        }
        renderListaProdutos();
    }
    ...
    function hydrateForm() {
        ...
        if (dom.precoYT1 && state.preco_y_t1 != null) dom.precoYT1.value = String(state.preco_y_t1).replace('.', ',');
        if (dom.precoYT2 && state.preco_y_t2 != null) dom.precoYT2.value = String(state.preco_y_t2).replace('.', ',');
        if (dom.regraVsTabela) dom.regraVsTabela.value = state.regra_vs_tabela || 'maior';
        if (!state.resolucoes_vs_tabela || typeof state.resolucoes_vs_tabela !== 'object') {
            state.resolucoes_vs_tabela = {};
        }
    ...
    function renderListaProdutos() {
        ...
        state.produtos.forEach(function (p, idx) {
            var pid = String(p.produto_externo_id || '');
            var regraItem = (state.resolucoes_vs_tabela && state.resolucoes_vs_tabela[pid]) || '';
            var precoPromoCell = valorDireto
                ? '<div class="grid grid-cols-3 gap-1">' +
                  '<input type="text" inputmode="decimal" data-idx="' + idx + '" data-campo="padrao" class="promo-preco-prod w-full rounded-lg border-2 border-emerald-200 px-1 py-2 font-black text-xs min-h-[40px]" placeholder="Padrão" value="' +
                  (p.preco_promocional != null ? String(p.preco_promocional).replace('.', ',') : '') +
                  '">' +
                  '<input type="text" inputmode="decimal" data-idx="' + idx + '" data-campo="t1" class="promo-preco-prod w-full rounded-lg border-2 border-orange-200 px-1 py-2 font-black text-xs min-h-[40px]" placeholder="T1" value="' +
                  (p.preco_promocional_t1 != null ? String(p.preco_promocional_t1).replace('.', ',') : '') +
                  '">' +
                  '<input type="text" inputmode="decimal" data-idx="' + idx + '" data-campo="t2" class="promo-preco-prod w-full rounded-lg border-2 border-orange-200 px-1 py-2 font-black text-xs min-h-[40px]" placeholder="T2" value="' +
                  (p.preco_promocional_t2 != null ? String(p.preco_promocional_t2).replace('.', ',') : '') +
                  '"></div>'
                : '<select data-regra-pid="' + pid + '" class="promo-regra-item w-full rounded-lg border-2 border-slate-200 px-2 py-2 text-[10px] font-black uppercase min-h-[40px]">' +
                  '<option value=""' + (!regraItem ? ' selected' : '') + '>Padrão promo</option>' +
                  '<option value="maior"' + (regraItem === 'maior' ? ' selected' : '') + '>Maior</option>' +
                  '<option value="promo"' + (regraItem === 'promo' ? ' selected' : '') + '>Promo</option>' +
                  '<option value="tabela"' + (regraItem === 'tabela' ? ' selected' : '') + '>Tabela %</option>' +
                  '</select>';
            html +=
        ...
        if (!valorDireto && n) {
            html =
                '<div class="px-3 py-2 flex flex-wrap gap-2 bg-orange-50 border-b border-orange-200">' +
                '<button type="button" class="promo-massa-regra text-[10px] font-black uppercase px-3 py-2 rounded-lg bg-slate-800 text-white" data-regra="maior">Todos → maior</button>' +
                '<button type="button" class="promo-massa-regra text-[10px] font-black uppercase px-3 py-2 rounded-lg bg-emerald-700 text-white" data-regra="promo">Todos → promo</button>' +
                '<button type="button" class="promo-massa-regra text-[10px] font-black uppercase px-3 py-2 rounded-lg bg-orange-600 text-white" data-regra="tabela">Todos → tabela %</button>' +
                '</div>' +
                html;
        }
        ...
        dom.lista.querySelectorAll('.promo-preco-prod').forEach(function (inp) {
            inp.addEventListener('change', function () {
                ...
                var campo = inp.getAttribute('data-campo') || 'padrao';
                if (!state.produtos[i]) return;
                var val = parseMoney(inp.value);
                if (campo === 't1') state.produtos[i].preco_promocional_t1 = val;
                else if (campo === 't2') state.produtos[i].preco_promocional_t2 = val;
                else state.produtos[i].preco_promocional = val;
            });
        });
        dom.lista.querySelectorAll('.promo-regra-item').forEach(function (sel) {
            sel.addEventListener('change', function () {
                var pid = sel.getAttribute('data-regra-pid');
                if (!pid) return;
                if (!state.resolucoes_vs_tabela) state.resolucoes_vs_tabela = {};
                var v = sel.value;
                if (!v) delete state.resolucoes_vs_tabela[pid];
                else state.resolucoes_vs_tabela[pid] = v;
            });
        });
        dom.lista.querySelectorAll('.promo-massa-regra').forEach(function (btn) {
            btn.addEventListener('click', function () {
                var r = btn.getAttribute('data-regra');
                if (!state.resolucoes_vs_tabela) state.resolucoes_vs_tabela = {};
                state.produtos.forEach(function (p) {
                    var id = String(p.produto_externo_id || '');
                    if (id) state.resolucoes_vs_tabela[id] = r;
                });
                renderListaProdutos();
            });
        });
    ...
    function coletarPayload() {
        ...
        return {
            ...
            preco_y_t1: dom.precoYT1 ? dom.precoYT1.value.trim() : null,
            preco_y_t2: dom.precoYT2 ? dom.precoYT2.value.trim() : null,
            regra_vs_tabela: dom.regraVsTabela ? dom.regraVsTabela.value : 'maior',
            resolucoes_vs_tabela: state.resolucoes_vs_tabela || {},
            ...
            produtos: state.produtos.map(function (p) {
                return {
                    ...
                    preco_promocional_t1: p.preco_promocional_t1,
                    preco_promocional_t2: p.preco_promocional_t2,
                };
            }),
        };
    }
