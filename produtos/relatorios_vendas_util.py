...
def _as_filtro_lista(filtro: object) -> list[str]:
    """Normaliza str/list/None; remove vazios e duplicados sem diferenciar caixa."""
    if filtro is None:
        return []
    valores = filtro if isinstance(filtro, (list, tuple, set)) else [filtro]
    out: list[str] = []
    vistos: set[str] = set()
...
def vendas_por_grupo_relatorio(
    desde: datetime,
    ate: datetime,
    *,
...
def vendas_por_grupo(
    ...
    """Contrato legado: agrupamento por categoria devolve somente a lista."""
    rows, _meta = vendas_por_grupo_relatorio(
        desde, ate, agrupar="categoria", deposito=deposito
    )
    return rows
