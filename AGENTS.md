...
- No PDV, botão **Pedir loja** (ao lado de Uso loja) abre overlay: busca + lista, envia pedido para a **outra loja** (Centro ↔ Vila). Saldo zerado/negativo **não impede**. 
- O pedido fica **pendente**. Na loja de origem o mesmo botão acende com **aviso (badge)**. Ações: Aceitar / Recusar / Transferir agora (PIN, baixa estoque Agro). Quem pediu pode **Cancelar** enquanto pendente. 
- A tela **Logística / Transferências** lista os mesmos pedidos (**Pedidos PDV**). Histórico entra em `HistoricoTransferencia` (`PEDIDO_PDV*`). Modelo: `estoque.SolicitacaoTransferenciaPdv`. APIs `/api/pdv/pedir-loja/`. 

**Entrada NF — assistente (7 passos)** 

- 1 Fornecedor · 2 Produtos · 3 Código de barras · **4 Lote e validade (opcional)** · 5 Estoque · 6 Financeiro · 7 Finalizar (PIN). Lote/validade: lista por item (como código de barras), dados do XML (`rastro` / `nLote`, `dFab`, `dVal`); checkbox «Pular lote e validade em todos os itens»; campos em `linhas[]` (`lote_numero`, `lote_fabricacao`, `lote_validade`, `lote_conf`).
