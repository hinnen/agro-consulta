...
def _static() -> None:
    ...
    chat_head = _read("produtos/templates/produtos/_wa_chat_head.html")
    status_tpl = _read("produtos/templates/produtos/_wa_status.html")
    html_form = html + composer + chat_head
    ...
    html_bot = _read("produtos/templates/produtos/atendimento_whatsapp_bot.html")
    js_bot = _read("produtos/static/produtos/js/atendimento_whatsapp_bot.js")
    check("html_vcf", "wa-vcf-inp" in html_bot)
    check("js_vcf", "agenda-vcf" in js_bot)
    check("html_sem_vcf_lista", "wa-vcf-inp" not in html)
    ...
    check("node_status_env", "enviarStatus" in node and "bridge/status/" in node)
    check("util_status_off", "@broadcast" in util and 'return None, "ignorado"' in util)
    check("util_status_proc", "def processar_status" in util and "def listar_status" in util)
    check("model_status", "class WhatsAppStatusAgro" in models)
    check("mig_0120", (ROOT / "produtos/migrations/0120_whatsapp_status.py").is_file())
    check("url_status", "api_atendimento_whatsapp_status" in urls)
    check("url_bridge_status", "api_atendimento_whatsapp_bridge_status" in urls)
    check("html_status_viewer", "_wa_status.html" in html and "wa-st-view" in status_tpl)
    check("html_chat_head", "_wa_chat_head.html" in html and "wa-topo-status" in chat_head)
    check("html_ficha_antes", html.find("_wa_ficha.html") < html.find("atendimento_whatsapp.js"))
    check("js_ficha_overlay", "abrirCadastroOverlay" in js and "AgroPdvOverlay" in js)
    check("js_ficha_fechar_deleg", "wa-ficha-fechar" in js and "closest" in js)
    check("js_status", "carregarStatus" in js and "wa-st-view" in js)
    check("js_status_stop", "pararMidiaStatus" in js)
    check("url_foto", "api_atendimento_whatsapp_foto" in urls)
    check("url_bridge_foto", "api_atendimento_whatsapp_bridge_foto" in urls)
    check("util_foto", "def gravar_foto_perfil" in util and "def listar_fotos_pendentes" in util)
    check("mig_0121", (ROOT / "produtos/migrations/0121_whatsapp_foto_perfil.py").is_file())
    check("node_foto", "profilePictureUrl" in node and "enviarFotoPerfil" in node)
    check("js_foto_lista", "foto_url" in js and "wa-av" in js)
    ...
    check("mig_0117", (ROOT / "produtos/migrations/0117_whatsapp_aviso_fora_em.py").is_file())
    ...
    js_pdv_wa = _read("produtos/static/produtos/js/pdv_topbar_whatsapp.js")
    check("pdv_wa_em_breve", "pdv-wa-em-breve" in html_pdv and "Em breve" in js_pdv_wa)
    check("pdv_wa_nao_abre_chat", "abrirChat" not in js_pdv_wa and "atendimento-whatsapp/estado" not in js_pdv_wa)
    check("url_excluir", "api_atendimento_whatsapp_excluir" in urls)
    check("url_concluir", "api_atendimento_whatsapp_concluir" in urls and "concluir/" in urls)
    check("util_concluir", "def concluir_atendimento" in util)
    check("mig_0118", (ROOT / "produtos/migrations/0118_whatsapp_aguardando_loja.py").is_file())
    check("html_ok", 'id="wa-ok"' in chat_head)
    check("js_ok", "concluir/" in js and "has-wait" in js)
    check("url_pairing", "api_atendimento_whatsapp_pairing" in urls)
    check("url_trocar", "api_atendimento_whatsapp_trocar" in urls and "trocar/" in urls)
    ...
    check("html_apagar", "wa-del" in chat_head)
    check("js_apagar", "atendimento-whatsapp/excluir/" in js)
    check("url_apagar_msg", "api_atendimento_whatsapp_apagar_mensagem" in urls and "apagar-mensagem/" in urls)
    check("util_apagar_msg", "def pedir_apagar_mensagem" in util)
    check("mig_0122", (ROOT / "produtos/migrations/0122_whatsapp_apagar_msg.py").is_file())
    check("js_apagar_msg", "apagar-mensagem/" in js and "data-apagar-msg" in js)
    check("node_apagar_msg", 'p.tipo === "apagar"' in node and "delete:" in node)
