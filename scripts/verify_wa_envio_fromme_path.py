...
def prova_estatico() -> None:
    ...
    check("function liberarEnvioUi" in js, "JS liberarEnvioUi")
    check("timeoutMs" in js, "JS timeout fetch enviar")
    check("type = 'button'" in js or 'type = "button"' in js, "JS forca send type=button")
    html_comp = read("produtos/templates/produtos/_wa_composer.html")
    check('id="wa-send"' in html_comp and 'type="button"' in html_comp, "HTML wa-send type=button")
