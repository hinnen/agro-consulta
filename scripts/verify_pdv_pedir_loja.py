...
def main() -> int:
    ...
    check("overlay_pc_largo", "64rem" in html)
    check("overlay_grid_pc", "pl-pedir-grid" in html and "900px" in html)
    check("overlay_abas_completas", "Recebidos" in html and "Histórico" in html)
    check("overlay_obs_visivel", 'id="pdv-pedir-loja-obs"' in html and "Observação (opcional)" not in html)
    ...
    check("version_bump", ver >= "16.55", ver)
