...
def main() -> None:
    ...
    check("lojas_sem_fiado", "vendas_lojas_sem_fiado_totais" in lojas)
    check("lojas_fiado_baixa", "vendas_lojas_fiado_baixas_periodo" in lojas)
    check("util_sem_fiado_abate", "abatimento_devolucoes_sem_fiado_totais_loja" in util)
    check("tpl_vl_fiado_tag", "vl-fiado-tag" in (ROOT / "produtos/templates/produtos/vendas_lojas_resumo.html").read_text(encoding="utf-8"))
    check("lojas_intervalo", "intervalo" in lojas and "dia_fim_iso" in lojas)
    tpl_vl = (ROOT / "produtos/templates/produtos/vendas_lojas_resumo.html").read_text(encoding="utf-8")
    check("tpl_vl_intervalo", "vl-goto-intervalo" in tpl_vl and "data_fim" in tpl_vl)
