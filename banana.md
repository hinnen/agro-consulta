...
- **DRE visual prévia (09/08, `DRE-VISUAL-PREVIA` + `DRE-VISUAL-POLISH`):** Resumo gerencial visual 16:9 (fluxo + donut despesas + receita PDV por categoria + **PE barras modernas** + mini DRE + empréstimos). Despesas por categoria = **mesmo recorte do card Despesas**. Mini DRE: Receita → CMV → Lucro bruto → Resultado op. → Líquido → **Juros empréstimo** → **Empréstimo** → **Saldo final** (= `geracao_caixa`, com empréstimos). Sem círculo de PE. Indicadores permanece até 100%. Empréstimos: devido/pago/juros = **filtro da tela**; emprestado = **sempre competência**.
