...
**Preço digitado + forma (`PDV-PRECO-MANUAL-FORMA` · v18.18 · 28/08):** preço editado no carrinho **não** volta ao lista ao escolher forma/recalc. Flag `preco_manual` + cache alinhado; campanha não sobrescreve. ? no `teste` (`523c06a`, prova **37/37**) · ? ainda **não** na loja.
...
| Venda **com impressÃ£o (F9)** + cupom fiscal | **Sim**, **sÃ­ncrono** â€” modal CPF/CNPJ â†’ aguarda SEFAZ â†’ imprime |
| Venda **sem impressÃ£o (Enter)** + PIX/cartÃ£o | **Sim**, background (sem modal se sem CPF/CNPJ no cliente) |
...
| ----- | ------------ | ----------------- |
| Confirmar **com impressÃ£o (F9)** | Modal CPF/CNPJ (se sem documento no cliente) â†’ emissÃ£o **sÃ­ncrona** â†’ imprime fiscal | Popup **Cupom fiscal** ou **Venda comum** â†’ idem se fiscal |
...
**UX PDV (2026-06-18):** modal CPF/CNPJ **grande** (`max-w ~54rem`, fontes `clamp`). ReemissÃ£o em `/vendas/` â†’ apÃ³s autorizar pergunta **Imprimir cupom / Agora nÃ£o**. Aviso pÃ³s-venda NFC-e falhou: toast **no topo**, depois da janela de impressÃ£o Windows.
...
- **Saudação + Resolvidas (`WA-SAUDACAO-RICH` + `WA-ARQUIVO` · 05/09 · v22.81):** Bot aba **Saudação** (não mais bloco cru no Menu) · **?** = arquivar ? aba **Resolvidas** · **Reabrir** · msg do cliente desarquiva · prefs auto em Bot ? **Arquivo** (default OFF, sem cron). Migrate **`0126`**.
...
| **2 â€” Mudar a tela LanÃ§amentos** | Contas a pagar/receber passam a **ler e gravar no Postgres Agro**, nÃ£o no Mongo | **ğŸ”¥ HOJE** â€” escopo **CP primeiro** |
...
| Foco | Detalhe |
| ---- | ------- |
| **ğŸ”¥ AGORA** | Renan â€” roteiro **7 passos** (CHECKPOINT) no **staging** |
...
| Foco | Detalhe |
| ---- | ------- |
| **ğŸ”¥ HOJE** | LanÃ§amentos PG â€” **CP em aberto** primeiro |
...
### ?? PACOTE PRONTO — Saudação completa + Resolvidas (`WA-SAUDACAO-RICH` + `WA-ARQUIVO` · **v22.81** · 05/09)
...
### ? CHECKLIST ÚNICO — pronto envio (05/09g · tip **v22.81**)
...
### ?? WA desligado (05/09 tarde) — PDV lento em todas as lojas
...
| **UI** | `WA-LISTA-SEM-PISCA` · `WA-FACHONA-PRETA` · `WA-PIN-COMPOSER` · `WA-SAUDACAO-RICH`/`WA-ARQUIVO` · **v22.81** · ?? pronto envio |
...
| **Status** | ? **Live v21.89** |
| **Você** | **Ctrl+F5** · janela anônima `/` ? login da marca |
...
| **Status** | ? **Live v21.89** |
| **Você** | Ctrl+F5 Entrada NF ? **Em andamento** sem digitar |

### ? Deploy loja — lote checklist 04/09b (`deploy/prep-checklist-0409` · **v21.89**) · **Live**

| Campo | Valor |
| ----- | ----- |
| **Status** | ? **enviado / Live v21.89** — cherry só o lote (**não** merge `teste`) · tip **`4910c79`** |
...
| **Você** | **Ctrl+F5** · badge **v21.89** · PDV consulta · F7 · BI `/` pede login · reiniciar `.bat` do Zap |

### ? CHECKLIST ÚNICO — 04/09b · **Live v21.89**
...
| **Você** | **Ctrl+F5** · reiniciar `.bat` · passar loja · pix · orçamento Loja |
...
| Campo | Valor |
| ----- | ----- |
| **Status** | ? **enviado / Live v21.87** — cherry só stack nest (**não** merge `teste`) · commit **`53b565a`** / tip docs **`55e9b6b`** |
...
| **Você** | frase+senha ? loja · Ctrl+F5 · Reemitir **1×** |
| **Risco** | Baixo |
...
| **Migrate** | **NÃO** |
| **Prova** | path 254 · overlay 165 — OK |
| **Você** | Ctrl+F5 Retiradas/PDV · Repasse · preencher 2 cofres + levar · Confirmar ? popup só confere |
| **Risco** | Baixo — mesma API `valor_cofre_*` + `valor_manual` |
...
### ~~? CHECKLIST ÚNICO — tip v19.37~~ · superado pelo tip **v19.47**

### ~~?? PACOTE PRONTO — tip v19.34 / v19.32~~ · superado pelo tip **v19.47**

### ~~? CHECKLIST ÚNICO — tip v19.34 / v19.33~~ · superado pelo tip **v19.47**

### ~~?? PACOTE PRONTO — tip v19.30 / v19.29~~ · ver tip **v19.47**

### ~~? CHECKLIST ÚNICO — tip v19.30~~ · superado pelo tip **v19.47**

### ?? PIN / Quem — bug report + Geraldo Hinnen (PIN-OPERADOR-QUEM · **v19.15** · 29/08/2026)

| Item | Detalhe |
| ---- | ------- |
| **Regra** | Sem PIN na sessão ? exige PIN · nunca login Chrome |
| **Status** | ver **CHECKLIST ÚNICO** tip v19.15 · prova **50/50** |
| **Você** | Ctrl+F5 · sem PIN = pede PIN · ?? nome do PIN |
...
### ~~?? ATENÇÃO producao antecipado / PREP 2808c~~ · **superado — Live v18.72 @ ae126d9**

### ? Deploy loja — lote checklist 28/08b (`deploy/prep-checklist-2808b` · **v18.64**) · **Live**

| Campo | Valor |
| ----- | ----- |
| **Status** | ? **enviado / Live v18.64** — `AGRO_APP_VERSION = "18.64"` · healthz **ok** · home/PDV/consulta **200** |
...
| **Migrate** | **SIM** — `produtos.0102` (choice `saldo_inicial`, SQL **no-op**) no build |
| **Rollback** | tag `rollback/pre-lote-checklist-2808b-v18.50` @ `4836ec1` · branch `producao-backup-pre-v1864-lote-checklist-20260828` · doc `docs/ROLLBACK-LOTE-CHECKLIST-2808b.md` · **só** frase + senha |
| **Você** | Ctrl+F5 nos PDVs · badge **v18.64**. Repasse overlay novo. Cofrinho: **Saldo inicial** se já tinha dinheiro físico. |

### ? CHECKLIST ÚNICO — enviado produção (28/08b · loja **v18.64**)
...
| **Não entrou** | Ver **PACOTE PRONTO / CHECKLIST ÚNICO** no topo (6 pacotes só no `teste`) |
...
### ??? Etiquetas — gôndola 60 × 30 mm (`ETQ-GONDOLA-6CM` · **v18.24** · 27/08)
...
| **??** | **NÃO** merge `teste`?`producao` · só esta branch · sem migrate |
...
| **??** | Branch isolada + migrate na loja · **não** merge `teste` inteiro |
...
| **??** | Subir **sobre loja v13.83** com patch isolado do modal — **não** o modal completo do `teste` (tem WIP) |
...
### ?? Lembrete — NÃO subir 	este inteiro
...
### ğŸ› Dist DF-e — caixa de entrada salva (~80) (31/07)
...
### ğŸ› Dist DF-e — 656 avançava o NSU sem puxar nota (31/07)
...
### ğŸ› Dist DF-e — certificado na branch `teste` (30/07)
...
### ğŸ› Dist DF-e — certificado «não configurado» no local (29/07)
...
- **«v9.92» kardex e-mail + ?camada** — **já na loja desde v10.63** (`1e6fe3c`); checklist antigo enganava.
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” PDV Pix mÃ¡quina Sicredi (`PDV-PIX-SICREDI` Â· **v11.94**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” Folha saldo presets + transferÃªncia sem saldo+ (`FOLHA-TRANSF` Â· **v11.93**)
...
### ğŸ“¦ PACOTE â€” Folha saldo presets + transferÃªncia sem saldo+ (28/07)
...
### ğŸ› SEFAZ Dist DF-e â€” cStat 656 na 1Âª consulta do dia (27/07)
...
### ğŸ©¹ FIX â€” Ajuste Mobile saldo Â«tudo 0Â» / filtro sÃ³ positivo (27/07)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” Cadastro filtros + Folha saldo + Ajuste mobile (`CAD-COMPRAS-AJUSTE` Â· 27/07)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **pronto p/ prÃ³ximo chat** â€” push `teste` Â· **nÃ£o** produÃ§Ã£o ainda Â· espera pausa loja + frase + senha `99738595` |
...
### ğŸ©¹ FIX â€” `/api/pdv/saldos/` 500 local (27/07)
...
### ğŸŒ Local lento â€” Postgres Oregon (27/07)
...
### ğŸ› FIX â€” Folha Compras Â«por fornecedorÂ» sem Mongo (27/07)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” Dispenser +13 sabores (`DSP-SABORES` Â· 24/07)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **pronto** Â· **NÃƒO subir** enquanto loja vende Â· prÃ³ximo chat: pausar vendas + frase + senha `99738595` |
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” HistÃ³rico estoque / kardex ledger (**v11.68** Â· 23/07)
...
### ğŸ“Œ Reaplicar custos NF em lote `--todas` (**teste v11.65** Â· 22/07)
...
### ğŸ§­ Teste = LOCAL (22/07 Â· Renan Â· opÃ§Ã£o B)
...
### ğŸ› Cadastro BCA â€” Â«lista localÂ» incompleta (**teste v11.60**)
...
### ğŸ”’ Cadastro BCA â€” sÃ³ servidor (**teste v11.61**)
...
### ğŸ”’ PDV â€” lista nÃ£o cresce depois (**teste v11.62**)
...
### ğŸš‘ PDV â€” lag 1â€“2s em tudo (**teste v11.64**)
...
### ğŸ› DevoluÃ§Ã£o loja â€” Â«Falha de redeÂ» (#3798 Centro) (**teste v11.58**)
...
### ğŸ”ª Corte Mongo na Entrada NF (**teste v11.57**)
...
### ğŸ› Entrada NF â€” custo nÃ£o atualiza no Cadastro (**teste v11.54**)
...
### ğŸš¨ URGENTE â€” Mongo ERP Authentication failed (**teste v11.53**)
...
### ğŸ“¦ Pacote local BCA â€” ordem A (**teste v11.52**)
...
### ğŸš‘ Import Mongoâ†’PG sÃ³ faltantes (22/07 Â· **loja v11.04**)
...
### ğŸ“‹ CHECKLIST decisÃ£o â€” pacote/BCA definitivo (22/07 Â· conversa Renan)
...
### ğŸ©¹ Recuperar produtos de itens de venda (22/07 Â· **teste**)
...
### ğŸ©¹ Cadastro â€” foto Delivery sumia + PDV (21/07 Â· **teste v11.45**)
...
### ğŸš€ CatÃ¡logo â€” famÃ­lia de embalagens granel+sacos (21/07 Â· **teste**)
...
### ğŸ“¦ Contagem â€” data Ãºltimo AJUSTE PIN no mobile + PDV (21/07 Â· **teste v11.40**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” Entrada NF Â«NovaÂ» limpa (**v10.90**)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **PRONTO PARA ENVIO PARA PRODUÃ‡ÃƒO** |
...
### ğŸ©¹ Entrada NF â€” Nova herdava XML/dados da nota anterior (21/07 Â· **teste v11.38**)
...
| **Arquivo** | `entrada_nota.html` |
| **Status** | ğŸ“¦ **PRONTO PARA ENVIO PARA PRODUÃ‡ÃƒO** Â· pacote **v10.90** |
| **VocÃª** | Ctrl+F5 Â· abrir nota Â· **Nova** Â· deve nascer em branco Â· puxar XML novo sem rastro da anterior |

### ğŸ©¹ Entrada NF â€” Mudar produto baixava V. unit com acrÃ©scimos (21/07 Â· **teste**)
...
### ğŸª Ajuste mobile â€” depÃ³sito inicial = loja do PDV (21/07 Â· **teste**)
...
### ğŸ” Ajuste mobile â€” PIN a cada abertura + operador nos ajustes (21/07 Â· **teste**)
...
### ğŸ©¹ Ajuste mobile â€” lista sÃ³ 7 produtos (21/07 Â· **teste**)
...
### ğŸ©¹ Postgres slots â€” leak BI threads (21/07 Â· **teste**)
...
### ğŸ” Incidente manhÃ£ 21/07 â€” site nÃ£o abria **antes** do deploy (confirmaÃ§Ã£o Renan)
...
### ğŸ©¹ Caixa â€” diferenÃ§a abertura + quem abriu/fechou (21/07 Â· **teste**)
...
### ğŸ©¹ Abrir caixa â€” botÃ£o CÃ©dulas (21/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v10.87** â€” Caixa VilaÃ—Centro + conn (21/07 Â· Renan)
...
### ğŸ©¹ LogÃ­stica/TransferÃªncias â€” abertura lenta (21/07 Â· teste)
...
### ğŸ“¦ Deploy loja **v10.86** â€” Caixa Vila Ã— Centro (21/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.82** â€” Hotfix entregas PIN + Imprimir (19/07 Â· Renan frase+senha)
...
### ğŸ©¹ PDV â€” Entregas nÃ£o clicÃ¡veis sob Modo descanso/PIN (19/07 Â· **teste**)
...
### ğŸ©¹ PDV â€” popup Imprimir atrÃ¡s do overlay entregas (19/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v10.81** â€” Assumir entrega catÃ¡logo CentroÃ—Vila (19/07 Â· Renan frase+senha)
...
### ğŸš€ PDV â€” Assumir entrega catÃ¡logo CentroÃ—Vila (19/07 Â· **teste v10.82** Â· **bce06da**)
...
### ğŸ“¦ Deploy loja **v10.80** â€” CatÃ¡logo sem texto extra do WhatsApp (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.67** â€” CatÃ¡logo WA 1Âº + OG delivery raÃ§Ã£o (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.66** â€” OG card 1200Ã—630 sem cortar logo (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.65** â€” CatÃ¡logo preview WhatsApp / OG (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.64** â€” Renan sem NFC-e auto + fechar ordem (18/07)
...
### ğŸ©¹ Fechar caixa â€” ordem fixa das formas + sem Fiado (18/07 Â· **teste v10.69**)
...
### ğŸ©¹ PDV â€” Mercado Pago Renan sem NFC-e automÃ¡tica (18/07 Â· **teste**)
...
### ğŸ©¹ PDV â€” Mercado Pago Renan sem NFC-e automÃ¡tica (18/07 Â· **teste v10.68**)
...
### ğŸ“¦ Deploy loja **v10.63** â€” kardex + C1â€“C3 + Zap legado (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” v10.63 kardex + C1â€“C3 + Zap legado (18/07)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **commit pronto** 1e6fe3c Â· tag rollback 
...
### ğŸ“¦ Deploy loja **v10.62** â€” CatÃ¡logo GPS checkout (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.61** â€” aba 9 histÃ³rico sÃ³ o que mudou (18/07)
...
### ğŸ“¦ Deploy loja **v10.60** â€” Compras UI etapa 1 (18/07)
...
### ğŸ“¦ Deploy loja **v10.59** â€” BI mensagem + validade + faturamento 2 barras (18/07)
...
### ğŸ©¹ BI Vila â€” top clientes + validade + faturamento (18/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v10.58** â€” CatÃ¡logo delivery (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.57** â€” FL-024 picklist (18/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v10.56** â€” BI Vila zeros + trava caixa (18/07 Â· Renan frase+senha)
...
### ğŸ” PrÃ©-produÃ§Ã£o â€” revisÃ£o catÃ¡logo + divergÃªncia (18/07 Â· este chat)
...
### ğŸš¨ Caixa â€” trava fechado + loja cruzada (18/07 Â· **teste**)
...
### ğŸš¨ Retiradas â€” bloqueio com caixa fechado (18/07 Â· **teste**)
...
### ğŸ©¹ BI Vila â€” zerar cards que vazavam Centro (18/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v10.48** â€” BI + retiradas por loja (18/07 Â· Renan frase+senha)
...
### ğŸ©¹ Retiradas/saÃ­das â€” filtro por loja Centro/Vila (18/07 Â· **teste**)
...
### ğŸ©¹ BI â€” seletor loja filtra nÃºmeros + recarrega (18/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v10.28** â€” pacote Vila Elias / Centro (18/07 Â· Renan frase+senha)
...
### ğŸ” RevisÃ£o risco pacote Vila â†’ CENTRO (18/07 Â· agente Opus)
...
### ğŸ©¹ Cadastro â€” saldo busca ficava velho apÃ³s venda (18/07 Â· **teste v10.23**)
...
### ğŸ©¹ Cadastro â€” fase 2 picklist (gestÃ£o / overlay / unidade / PDV) (18/07 Â· **teste v10.20**)
...
### ğŸ©¹ Cadastro â€” marca/cat sÃ³ lista + PIN (FL-024) (18/07 Â· **teste v10.19**)
...
### ğŸ©¹ Cadastro â€” cÃ³digo sistema+GM no produto novo sem agro_pg (18/07 Â· **teste v10.17**)
...
### ğŸ©¹ PDV â€” badge estoque + aviso caixa sem F5 (18/07 Â· **teste v10.14**)
...
### ğŸ©¹ Caixa fechado â€” bloqueia venda + refresh no PDV (18/07 Â· **teste v10.11**)
...
### ğŸ©¹ Caixa no PDV â€” nÃ£o abrir BI no overlay (18/07 Â· **teste v10.10**)
...
### ğŸ©¹ Abrir caixa â€” seleÃ§Ã£o + digitar loja (18/07 Â· **teste v10.08**)
...
### ğŸ©¹ Caixa fechar â€” 500 (fiado_baixas_wizard) (18/07 Â· **teste v10.06**)
...
### ğŸ”’ Vila Elias â€” antiburro digitar loja (18/07 Â· **teste v10.04**)
...
### ğŸ’° Vila Elias â€” Caixa separado Centro Ã— Vila (18/07 Â· **teste v10.03**)

| Item | Detalhe |
| ---- | ------- |
| **Status** | âœ… push `teste` Â· validar no Render Â· ğŸ“¦ produÃ§Ã£o sÃ³ com frase + senha |
...
### ğŸª Vila Elias â€” PDV baixa estoque por loja do aparelho (18/07 Â· **teste v10.02**)
...
### ğŸ©¹ Entrada NF â€” histÃ³rico C1â€“C3 nÃ£o duplica a prÃ³pria nota (18/07 Â· **teste v9.97**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” Compras etapa 1 UI (18/07 Â· **teste v9.95+**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” aba 9 histÃ³rico sÃ³ o que mudou (18/07 Â· **teste v10.01**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” kardex e-mail + Entrada NF Î”camada (18/07 Â· **teste v9.92**)
...
### ğŸ“¦ Deploy loja **v9.91** â€” kardex Quem + Entrada NF (18/07 Â· Renan frase+senha)
...
### ğŸ©¹ Cadastro â€” kardex Quem + Entrada NF (17/07 Â· **teste v9.91** Â· **loja v9.91**)
...
### ğŸ“¦ Deploy loja **v9.90** â€” Fecha PDV+Cadastro+NFC-e+RelatÃ³rios (17/07 Â· Renan frase+senha)
...
### ğŸ“¦ PDV â€” Enviar orÃ§amento WhatsApp Â· pronto produÃ§Ã£o (17/07 Â· **teste v9.90**)
...
### ğŸ©¹ PDV â€” Enviar Zap tambÃ©m grava em OrÃ§amentos (17/07 Â· **teste v9.89**)
...
### ğŸ©¹ PDV â€” texto do aviso central maior (17/07 Â· **teste v9.88**)
...
### ğŸ©¹ PDV â€” avisos Enviar Zap no meio da tela (17/07 Â· **teste v9.87**)
...
### ğŸ©¹ PDV â€” avisos Enviar Zap sem alert Chrome (17/07 Â· **teste v9.86**)
...
### ğŸ©¹ PDV â€” texto Zap orÃ§amento padrÃ£o Agromais (17/07 Â· **teste v9.85**)

| | |
| --- | --- |
| **O quÃª** | Mensagem WhatsApp no formato do 2Âº print: *ORÃ‡AMENTO AGROMAIS*, item em 1 linha 1x - nome  R$, linhas finas, sem losango/ğŸ’° |
...
### ğŸ©¹ PDV â€” some azul de vez (painel+campo) (17/07 Â· **teste v9.83**)
...
### ğŸ©¹ PDV â€” some de vez a linha/faixa azul da busca (17/07 Â· **teste v9.82**)
...
### ğŸ©¹ PDV â€” tira linha azul sob a busca (17/07 Â· **teste v9.81**)
...
### ğŸ©¹ PDV â€” Ã­cone Cliente fora do campo + texto Carrinho (17/07 Â· **teste v9.80**)
...
### ğŸ©¹ PDV â€” sÃ³ Ã­cones sem texto (teste visual) (17/07 Â· **teste v9.79**)
...
### ğŸ©¹ PDV â€” Cliente/Busca/Carrinho mesmo padrÃ£o (17/07 Â· **teste v9.78**)
...
### ğŸ©¹ PDV â€” busca +1 ponto (17/07 Â· **teste v9.77**)
...
### ğŸ©¹ PDV â€” busca fonte maior de novo (17/07 Â· **teste v9.76**)
...
### ğŸ©¹ PDV â€” fonte digitada = Busca (17/07 Â· **teste v9.75**)
...
### ğŸ©¹ PDV â€” busca um pouco maior (17/07 Â· **teste v9.74**)
...
### ğŸ©¹ PDV â€” busca na linha do Ã­cone Busca (17/07 Â· **teste v9.73**)
...
### ğŸ©¹ PDV â€” reverte busca na linha do Cliente (17/07 Â· **teste v9.72**)
...
### ğŸ©¹ PDV â€” reverte busca no header (17/07 Â· **teste v9.71**)
...
### ğŸ©¹ PDV â€” Saldos sobe Â· botÃµes cliente Ã  esquerda (17/07 Â· **teste v9.67**)
...
### ğŸ©¹ PDV â€” tira Etapa 1/Produtos Â· ? sobe pro header (17/07 Â· **teste v9.66**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” PDV lÃ¡pis â†’ aba 9 AlteraÃ§Ãµes (17/07 Â· âœ… enviado v9.90)

| Item | Detalhe |
| ---- | ------- |
| **Status** | âœ… **testado no teste** Â· ğŸ“¦ **pronto pra envio** â€” espera frase + senha `99738595` na **mesma mensagem** |
...
### ğŸ©¹ PDV + Cadastro â€” histÃ³rico aba 9 tambÃ©m no lÃ¡pis (17/07 Â· **teste v9.62**)
...
| **Status** | ğŸ“¦ **pronto pra envio** â€” pacote acima |
| **VocÃª** | Ctrl+F5 Â· badge **v9.62** Â· lÃ¡pis muda nome/preÃ§o â†’ cadastro â†’ aba 9 Â· confere origem |

### ğŸ©¹ PDV etapa 1 â€” layout: tira ?, botÃµes no cliente, busca sobe (17/07 Â· **teste v9.60**)
...
| **Pacote lÃ¡pis** | Continua ğŸ“¦ pronto; este layout sobe junto se pedir PDV |

### ğŸ“¦ PACOTE PRONTO LOJA â€” Cadastro modal editar (UX + histÃ³ricos) (17/07 Â· âœ… enviado v9.90)
...
### ğŸ©¹ Cadastro â€” PreÃ§os: nÃºmero grande de verdade (17/07 Â· **teste v9.56**)

| Item | Detalhe |
| ---- | ------- |
| **O quÃª** | Custo / MVA / comissÃ£o / margem: fonte **~2,75rem** no nÃºmero. Caixa sem altura artificial. PreÃ§o final **~3rem** |
| **Status** | ğŸ“¦ fecha no pacote **Cadastro modal** acima |
| **VocÃª** | Ctrl+F5 Â· badge **v9.56** Â· aba PreÃ§os |

### ğŸ©¹ Cadastro â€” Fiscal: fonte maior + campos mais juntos (17/07 Â· **teste v9.55**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” PDV editor rÃ¡pido (lÃ¡pis) (17/07 Â· âœ… enviado v9.90)
...
### ğŸ©¹ PDV â€” pente fino editor rÃ¡pido (17/07 Â· **teste v9.54**)

| | |
| --- | --- |
| **O quÃª** | Lazy unidade Â· Esc correto Â· custo no patch do carrinho Â· scroll sÃ³ se Formas A/B estourar tela baixa |
| **Status** | ğŸ“¦ fecha no pacote acima |

### ğŸ©¹ PDV â€” editor sem scroll: pop quase tela cheia (17/07 Â· **teste v9.53**)
...
### ğŸ©¹ PDV â€” editor rÃ¡pido bem maior (+25 %) (17/07 Â· **teste v9.51**)
...
### ğŸ©¹ Cadastro â€” PreÃ§os bem legÃ­vel + preenche altura + URL imagem (17/07 Â· **teste v9.50**)
...
### ğŸ©¹ PDV â€” editor rÃ¡pido um degrau maior (17/07 Â· **teste v9.49**)
...
### ğŸ©¹ Cadastro â€” modal: proporÃ§Ã£o PreÃ§os + listas atÃ© o rodapÃ© (17/07 Â· **teste v9.48**)
...
### ğŸ©¹ PDV â€” estoque editor: nÃºmeros do mesmo tamanho (17/07 Â· **teste v9.46**)
...
### ğŸ©¹ PDV â€” editor: 2 tags estoque + unidade busca (17/07 Â· **teste v9.44**)
...
### ğŸ©¹ PDV â€” editor rÃ¡pido puxa GM/barras/custo certos (17/07 Â· **teste v9.41**)
...
### ğŸ©¹ PDV â€” formas A/B recolhidas no editor rÃ¡pido (17/07 Â· **teste v9.40**)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” RelatÃ³rios ajuda Â«?Â» (17/07 Â· âœ… enviado v9.90)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **pronto pra envio** â€” Renan OK no teste Â· espera frase + senha `99738595` na **mesma mensagem** |
...
| **Loja** | ğŸ“¦ **pronto pra envio** (pacote RelatÃ³rios ajuda Â«?Â») |
...
| **Loja** | ğŸ“¦ **pronto pra envio** â€” ver pacote **RelatÃ³rios ajuda Â«?Â»** |
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” NFC-e FL-056 (17/07 Â· âœ… enviado v9.90)

| Item | Detalhe |
| ---- | ------- |
| **Status** | ğŸ“¦ **pronto pra envio** â€” espera frase + senha `99738595` na **mesma mensagem** |
...
### ğŸ› NFC-e â€” rejeiÃ§Ãµes 963 + 225 (17/07 Â· teste)
...
| **Status** | ğŸ“¦ **pronto pra envio** (loja ainda v9.16) |
| **Loja** | frase + senha na mesma mensagem |

### ğŸ“¦ Deploy loja **v9.16** â€” Fecha + RelatÃ³rios + NFC-e #23 (17/07 Â· Renan frase+senha)
...
### ğŸ“¦ PACOTE PRONTO LOJA â€” âœ… ENVIADO v9.16 (17/07)
...
| **#7** notebook impressÃ£o | Ainda ğŸ”´ aberto |
...
### ğŸ“¦ Deploy loja **v9.14** â€” RelatÃ³rios (pÃ³s-v9.07) (16/07 Â· Renan frase+senha)
...
### ğŸ“¦ Deploy loja **v9.04** â€” Central de RelatÃ³rios (16/07 Â· Renan frase+senha)
...
| Item | Detalhe |
| ---- | ------- |
| **Status** | âœ… **testado no teste** Â· **ğŸ“¦ pronto para envio Ã  produÃ§Ã£o** (frase + senha) |
...
| **P1** | Kardex | E-mail no Quem + Entrada NF ?camada | ? **já loja v10.63** |
...
| **P1,1** | **FL-029** | Baixa parcial fiado + crédito | ?? **parcial**: baixa ? loja v7.61 · crédito ? ver **FL-058 P0,2** |
...
| **P1,9** | **FL-034** | Histórico F8 filtra cliente | ?? **código filtro na loja** — confirmar se fecha o pedido |
...
### ğŸ“¦ Deploy loja **v8.69** â€” Entrada NF busca BCA (16/07 Â· Renan frase+senha)
...
### ğŸ©¹ Entrada NF â€” busca BCA = cadastro (16/07 Â· **teste v8.94** Â· **loja v8.69**)
...
### ğŸ“Š Central de RelatÃ³rios â€” pacote completo (16/07 Â· teste)
...
### ğŸ“¦ Pacote pronto loja â€” fechar depois (16/07 Â· Renan)
...
### ğŸ©¹ Cupom â€” risco sumia na impressÃ£o (16/07 Â· **teste v8.89** Â· **âœ… Renan**)
...
### ğŸ©¹ Cupom â€” risco no item devolvido (16/07 Â· **teste v8.85**)
...
### ğŸ©¹ #17 busca valor progressiva (16/07 Â· **teste**)
...
### ğŸ©¹ #17 busca CP â€” valor/parcela sem lixo (16/07 Â· **teste v8.84**)
...
| **Loja** | ğŸ“¦ **pronto produÃ§Ã£o** â€” frase + senha (pacote do fecha c/ #12) |
...
| **Validar** | âœ… Renan Â· **ğŸ“¦ pronto loja (fecha)** |

### ğŸ“¦ Deploy loja **v8.68** â€” FL-021 hotfix match NF (16/07 Â· Renan frase+senha)
...
### ğŸ©¹ CP â€” botÃ£o NF ainda vazio (FL-021 hotfix) (16/07 Â· **teste v8.76** Â· **loja v8.68**)
...
### ğŸ©¹ DevoluÃ§Ã£o â€” textos no ? (16/07 Â· **teste v8.74**)
...
### ğŸ©¹ DevoluÃ§Ã£o â€” fontes do modal um pouco maiores (16/07 Â· **teste v8.72**)
...
### ğŸ©¹ DevoluÃ§Ã£o â€” confirmaÃ§Ã£o bonita + valor igual aos itens (16/07 Â· **teste v8.70**)
...
### ğŸ©¹ DevoluÃ§Ã£o â€” modal maior (idosos) (16/07 Â· **teste v8.69**)
...
### ğŸ“¦ Deploy loja **v8.67** â€” botÃ£o NF no CP (FL-021) (16/07 Â· Renan frase+senha)
...
### ğŸ©¹ CP â€” botÃ£o NF sumiu na lista (FL-021) (16/07 Â· **teste v8.67** Â· **loja v8.67**)
...
### ğŸ“¦ Deploy loja **v8.65** â€” preÃ§os A/B + PDV (16/07 Â· Renan frase+senha)
...
### ğŸ©¹ PDV â€” PAGAR sem travar cliente + consumidor na busca (15/07 Â· **teste v8.64**)
...
### ğŸ©¹ PDV â€” cliente Esc + valor grupo A/B (15/07 Â· **teste v8.63**)
...
### ğŸ©¹ PDV carrinho â€” colunas fixas + total sem corte (15/07 Â· **teste v8.60**)
...
### ğŸ©¹ PDV â€” alinhamento GM + 2 preÃ§os (lista e carrinho) (15/07 Â· **teste v8.59**)
...
### ğŸ©¹ PDV â€” grupos A/B aparecem + modal preÃ§os maior (15/07 Â· **teste v8.57**)
...
### ğŸ©¹ Cadastro â€” busca digitada mais leve (15/07 Â· **teste v8.55**)
...
### ğŸ“¦ Deploy loja **v8.54** â€” Entrada NF UX restante (15/07 Â· Renan frase+senha)
...
### ğŸ©¹ Entrada NF â€” alinhamento linha + borda campos (15/07 Â· **teste v8.54**)
...
### ğŸ©¹ Entrada NF â€” emb. recolhida alarga CÃ³d./EAN (15/07 Â· **teste v8.53**)
...
### ğŸ©¹ Entrada NF â€” nome do cadastro sob a descriÃ§Ã£o (15/07 Â· **teste v8.51**)
...
### ğŸš‘ Entrada NF â€” overlay R$ 0 apagava P.venda do Mongo (15/07 Â· **teste v8.49**)
...
### ğŸ“¦ Deploy loja **v8.49** â€” UX Entrada NF + P.venda (15/07 Â· Renan frase+senha)
...
### ğŸ©¹ Entrada NF â€” abas/meta na barra do topo (15/07 Â· **teste v8.48**)
...
### ğŸ©¹ Entrada NF â€” colunas embalagem recolhÃ­veis (15/07 Â· **teste v8.47**)
...
### ğŸ©¹ Entrada NF â€” abas + meta numa linha (15/07 Â· **teste v8.46**)
...
### ğŸ©¹ Entrada NF â€” UX rateio + passos 1â€“8 numa linha (15/07 Â· **teste v8.45**)
...
### ğŸ“¦ Deploy loja **v8.43** â€” rateio frete/ST + hidrata GM paralelo (14/07 Â· Renan frase+senha)
...
### ğŸš‘ Entrada NF â€” apÃ³s Confirmar XML sem GM / P.venda 0 (14/07 Â· **loja v8.41**)
...
### ğŸ©¹ Entrada NF â€” modal XML Â«Sem vÃ­nculoÂ» com grade vazia (14/07 Â· **loja v8.40**)
...
### ğŸš‘ Hotfix loja **v8.39** â€” merge migraÃ§Ãµes 0050+0051 (14/07)
...
### ğŸ“¦ Deploy loja **v8.38** â€” lote Zap restante + PIN ao abrir (14/07 Â· Renan frase+senha)
...
### ğŸ” PDV pede PIN ao abrir (14/07 Â· **teste v8.37** â†’ **loja v8.38**)
...
### ğŸ“¦ Deploy loja **v8.34** â€” #6 + #10 + PIN nome (14/07 Â· Renan frase+senha)
...
### ğŸ‘ PDV â€” nome do PIN entre Nova venda e PIN (14/07 Â· **teste v8.34**)
...
### ğŸš‘ PIN / nome trocado (14/07 Â· **teste v8.33**)
...
### ğŸ” PIN / nome trocado (Renan â†” Geraldo etc.) â€” 14/07 Â· **sÃ³ diagnÃ³stico**
...
### ğŸ©¹ #6 PIN â€” 1 query no rÃ³tulo (14/07 Â· **teste**)
...
### ğŸ“¦ Deploy loja **v8.30** (14/07 Â· Renan frase+senha)
...
### ğŸš‘ Fiado BAIXA Â«Nenhum tÃ­tulo em abertoÂ» (14/07)
...
### ğŸš‘ Hotfix #3 GM0024 loja sÃ³ 1 (14/07 Â· **v8.28**)
...
### ğŸ“¦ Deploy loja **v8.26** â€” busca GM CodigoNFe (14/07 Â· Renan autorizou)
...
### ğŸš‘ #3 busca GM â€” causa real + teste local (14/07)
...
### ğŸš‘ Hotfix #3 GM maiÃºsc/minÃºsc + famÃ­lia (14/07 Â· **v8.25**)
...
### ğŸš‘ Hotfix migraÃ§Ã£o loja (14/07) â€” deploy v8.22 falhou no Render
...
### ğŸ“¦ Deploy loja **v8.22** â€” pacote #3 + #4 (14/07 Â· Renan frase+senha)
...
### ğŸ› Hotfix #3 busca prefixo GM (teste **v8.22** Â· **loja v8.22**)
...
### ğŸ› Hotfix #4 modelo persiste (teste **v8.19+** Â· **loja v8.22**)
...
### ğŸ”„ Handoff Renan 14/07 â€” histÃ³rico (versÃ£o antiga)
...
### ğŸ› Pacote performance + UX (teste **v8.17**)
...
### ğŸ› Hotfix #18 frete nas 3 vias e cupom fiscal (teste **v8.16**)
...
### ğŸ› Hotfix #3 custo via BCA (teste **v8.15**)
...
### ğŸ› PÃ³s-validaÃ§Ã£o bugs loja Zap 12/07 (teste **v8.14**) â€” ajustes amarelo/vermelho
...
### ğŸ› Lote bugs loja Zap 12/07 (teste **v8.12**) â€” pacote checklist #
...
### ğŸš€ Deploy loja **v8.07** â€” BCA + pacote teste (11/07 â€” Renan senha OK)
...
### ğŸ› FIX â€” Indicadores faturamento PDV Â«â€”Â» (11/07 Â· **v8.04 teste**)
...
### ğŸ› FIX â€” Indicadores financeiros 500 (09/07 Â· **v7.54**)
...
### ğŸ› FIX â€” Indicadores 500 + despesas fixa/variÃ¡vel (09/07 Â· **v7.54â€“v7.55**)
...
### ğŸ› FIX â€” Compras nÃºmeros zerados no teste (08/07 Â· Renan Â· v7.51)
...
### ğŸ› FIX â€” barra lateral sumiu (08/07 Â· Renan Â· v7.47)
...
### ğŸ› REVERT â€” barra lateral BI (08/07 Â· Renan)
...
### ğŸ› HOTFIX â€” barra lateral nÃ£o monta (08/07 Â· Renan)
...
### ğŸ› HOTFIX â€” Compras mÃ©tricas zeradas + barra lateral (08/07 Â· Renan)
...
### ğŸ› HOTFIX â€” barra lateral / navegaÃ§Ã£o (08/07 Â· Renan)
...
### ğŸ› Compras â€” Folha Compras popup (08/07)
...
### ğŸ› Cadastro ERP â€” marca/categoria sumindo (08/07)
...
### ğŸ“‹ DecisÃ£o â€” popups / modais (08/07 Â· Renan)
...
### ğŸ› NFC-e â€” SEFAZ conexÃ£o recusada (07/07)
...
### ğŸ› PDV v7.19 (07/07) â€” fluxo entrega
...
### ğŸ”Œ MP Point â€” reativar conta nova (06/07)
...
| **Status** | **ğŸ§ª teste v6.87** deploy |
...
### ğŸ› PDV â€” botÃ£o PIN nÃ£o abre (05/07 noite)
...
| **Status** | **ğŸ§ª teste v6.86** |

### ğŸ› Caixa â€” contagem some ao fechar navegador (05/07 noite)
...
### ğŸ› PDV â€” descanso atrÃ¡s do modal Entregas (05/07)
...
| **Status** | **ğŸ§ª teste v6.85** â€” validar pagamento, NFC-e, cliente, entrega |

### ğŸ› Entregas PDV vazio mas bloqueia fechar caixa (05/07)
...
| **Status** | **ğŸ§ª teste v6.85** â€” validar botÃ£o laranja + lista com caixa |

### ğŸ› Caixa fechar â€” cache contagem sumindo (05/07)
...
| **Status** | **ğŸ§ª teste v6.81** |

### ğŸ› Caixa fechar â€” tela trava com modal cÃ©dulas + descanso (05/07)
...
| **Status** | **ğŸ§ª teste v6.81** â€” validar fechamento + contagem cÃ©dulas (PDV overlay e pÃ¡gina direta) |
...
| **Status** | **ğŸ§ª teste** Â· **nÃ£o estava na loja** (v6.77 sÃ³ tinha fix CPF/F9 no PDV) |
...
### ~~ğŸ“‹ Fila â€” **FL-050**~~ *(fechado â€” ver acima)*

### ~~ğŸ“‹ Fila â€” **FL-049**~~ *(fechado â€” ver acima)*
...
### ğŸ› Fechar caixa â€” contagem do dia anterior (03/07)
...
### ğŸ› Fiado â€” busca sÃ³ com saldo (03/07)
...
### ğŸ› Modo descanso â€” tela borrada ilegÃ­vel (03/07 Â· loja)
...
### ğŸ› RH folha â€” vale duplicava pagamento CP (02/07 Â· Renan) â€” **âœ… loja v6.20**
...
### ğŸ› FL-048 â€” Â«Baixar ZIP selecionadoÂ» nÃ£o baixava (02/07 Â· Renan)
...
### ğŸ› PDV wizard â€” carrinho itens travados (02/07 Â· Renan) â€” **âœ… loja v6.16**
...
### ğŸ› PDV wizard â€” botÃ£o **Pagar** clique morto Â· **âœ… loja v6.15**
...
### ğŸš€ Deploy loja **v6.14** â€” FL-048 kit + env no ZIP + backup noturno (30/06)
...
### ğŸ› PDV topbar â€” ainda quebrado pÃ³s-v6.09 (02/07 madrugada Â· Renan)
...
### ğŸ› PDV overlay Caixa abre BI GestÃ£o (01/07 Â· Renan) â€” **âœ… v6.05**
...
### ğŸ› PDV topbar â€” Caixa / Vendas / Fiado nÃ£o abrem overlay (02/07 Â· Renan) â€” **fix teste v6.51 Â· loja ainda v6.09**
...
### ğŸ”§ Perf busca produtos â€” Entrada NF etapa 2 + Cadastro ERP (02/07 Â· Renan) â€” **âœ… v6.08**
...
### ğŸ”§ Fix â€” Â«Voltar ao PDV F1Â» some em GestÃ£o â€” **âœ… v6.05**
...
### ğŸ“‹ Pendente loja â€” **Win10 amanhÃ£ (02/07)** â€” 2 apps PDV + GestÃ£o na barra
...
### ğŸ› RH ficha â€” botÃ£o Â«Abrir folhaÂ» sumia (01/07 Â· teste)
...
### ğŸ”´ INCIDENTE loja â€” tela cinza / fila **01/07** (histÃ³rico)
...
### ğŸŸ  Fiado â€” sÃ³ 3 tÃ­tulos pelo PDV/F8 **v5.58â†’v5.62 loja** (01/07)
...
### ğŸŸ  F8 Â«Indisp.Â» na loja â€” fix catÃ¡logo Postgres **v5.55** (30/06)
...
### ğŸŸ¢ Staging perf F8 â€” resolvido **v5.53** (30/06)
...
### ğŸ§­ Dois trilhos â€” **nÃ£o misturar** (Renan Â· 30/06)
...
### ğŸ“¦ PACOTE LOJA â€” **v5.44** Â· **SUBIU** 30/06
...
**ğŸ›’ PDV â€” Relacionamento com cliente (F8 / botÃ£o Hist.)**
...
ğŸš€ **AtualizaÃ§Ãµes do Sistema â€” GM Agro** ğŸš€  
ğŸ“… **ProduÃ§Ã£o: 17 a 29 de junho**
...
â€¢ Menu **CAIXA** mais rÃ¡pido (detalhes pesados sÃ³ na tela **Saldo**) â€” **ğŸ“‹ fila** Â· prÃ³ximo deploy loja (ainda **nÃ£o** na v5.22 live).

**ğŸ§¾ Entrada de Nota Fiscal**  
...
**ğŸ’° LanÃ§amentos / Contas a pagar**  
...
| **Deploy loja** | **ğŸ“¦ Pacote v5.44 pronto** â€” aguardando autorizaÃ§Ã£o Renan (Â§ CHECKPOINT pacote loja) |
...
| **Renan 30/06** | **ğŸ“‹ Fila** â€” **nÃ£o** redeploy isolado agora Â· sobe **junto com o prÃ³ximo pacote** loja (fix `VERSION` no cherry-pick final) |
...
| Fase | Escopo | Status |
| ---- | ------ | ------ |
| **1** | Selo visual no carrinho PDV (wizard) | ğŸ”„ teste **v5.09** |
| **2** | Desconto promo no **DRE/relatÃ³rios** como Â«desconto clientesÂ» | ğŸ“‹ pendente |
| **3** | Linha de desconto promo na **impressÃ£o** da venda (80 mm / PDF) | ğŸ“‹ pendente |
...
| ------ | ----- | ---------- |
| **v5.44 Relacionamento F8** | Modal F8 + pets PG + fiado link | **âœ… teste** Â· **ğŸ“¦ pronto** Â· ver CHECKPOINT Â«PACOTE LOJA v5.44Â» |
...
| - | - | ------ | ------ | ------ | ----- |
| **FL-001** | **P3** | PreÃ§os / PDV | Tabelas de preÃ§o personalizÃ¡veis por **forma de pagamento** ou **grupo de cliente** | ğŸ“‹ Pendente | 29/06 |
| **FL-002** | **P3** | PromoÃ§Ãµes | Revisar **usabilidade** da tela de promoÃ§Ã£o e **limpar textos inÃºteis** | ğŸ“‹ Pendente | 29/06 |
| **FL-003** | **P2** | PromoÃ§Ãµes / PDV / DRE | **Fase 1** selo promo no carrinho PDV Â· **Fase 2** DRE Â«desconto clientesÂ» Â· **Fase 3** linha na impressÃ£o da venda | ğŸ”„ Fase 1 teste Â· 2â€“3 pendente | 29/06 |
| **FL-004** | **P3** | RH | **Batida de ponto** dos funcionÃ¡rios (registro entrada/saÃ­da) | ğŸ“‹ Pendente | 29/06 |
| **FL-005** | **P2** | Entrega / impressÃ£o | Na impressÃ£o (separaÃ§Ã£o/entrega): **valor em R$ do troco a levar** na ida â€” **conferir antes** (hoje sÃ³ Â«troco: sim/nÃ£oÂ») | ğŸ“‹ Pendente Â· ğŸ” conferir | 29/06 |
| **FL-006** | **P2** | PDV / Entregas | **Ligar PDV** ao painel de entregas + **revisÃ£o visual** da tela `/entregas/` | ğŸ“‹ Pendente | 29/06 |
| **FL-007** | **P2** | UX geral | Revisar **tamanhos de layout** (Agro Display Scale) â€” **comeÃ§ar por** `/vendas/` (consulta de vendas) | ğŸ“‹ Pendente | 29/06 |
| **FL-008** | **P1** | PDV | Itens no carrinho **travam** — não altera qtd, preço nem remove · ex. **GM6083** | ? **loja v11.99** | 28/07 |
| **FL-009** | **P2** | Etiquetas | Na tela de **impressÃ£o de etiquetas**: ao adicionar item, **nÃ£o fechar** o autocomplete (manter busca aberta para bipar/digitar o prÃ³ximo) | ğŸ“‹ Pendente | 29/06 |
| **FL-010** | **P2** | Vendas | **Consulta de vendas** (`/vendas/`): buscador e **filtros completos** (perÃ­odo, cliente, forma, status, textoâ€¦) | ğŸ“‹ Pendente | 29/06 |
| **FL-011** | **P3** | Cashback | Revisar tela de **cashback** â€” melhorar usabilidade | ğŸ“‹ Pendente | 29/06 |
| **FL-012** | **P2** | Entrada NF | **Lixeira** para **desvincular produto da nota** â€” alinhar escopo com **Queila** antes de codar | ğŸ“‹ Pendente Â· â“ Queila | 29/06 |
| **FL-013** | **P2** | Etiquetas / impressÃ£o | CÃ³digo de barras em **PNG** (tipografia atual dificulta bip **1D e 2D**) | ğŸ“‹ Pendente | 29/06 |
| **FL-014** | **P3** | PDV | Projetar forma **mais prÃ¡tica** de alterar **quantidade** no carrinho | ğŸ“‹ Pendente | 29/06 |
| **FL-015** | **P2** | Etiquetas / PDV | **Regra bipagem etiqueta granel** â€” PDV nÃ£o leu direito; Renan fez **poucos testes** ainda | ğŸ“‹ Pendente Â· ğŸ” validar | 29/06 |
...
| **FL-022** | **P1,1** | CP | **Busca** no campo de filtros **inconsistente** (resultados variam / nÃ£o acha) | âœ… **#17** Renan testou Â· ğŸ“¦ pronto produÃ§Ã£o (fecha) | 29/06 |
...
| **FL-029** | **P1,1** | Fiado | Baixa parcial ? loja v7.61 · **falta** opção deixar em **crédito** | ?? parcial | 11/07 |
| **FL-030** | **P1,3** | Fiado / PDV | Forma de **ignorar bloqueio** por cliente com **notinhas fiado vencidas** â€” **PIN Geraldo / Geraldinho** | ğŸ“‹ Pendente | 29/06 16:20 |
| **FL-031** | **P1,6** | Entregas | **Terminar** de arrumar tela **`/entregas/`** | ğŸ“‹ Pendente | 29/06 16:20 |
...
| **FL-052** | **P1,1** | Fiado / fiscal | **NFC-e na baixa fiado** â€” emitir cupom na **quitaÃ§Ã£o** com forma real (venda original `venda_agro`); validar contador/SEFAZ | ğŸ“‹ Fila apÃ³s **FL-051** | 07/07 |
| **FL-033** | **P3** | BI / Home | **Zap #21:** indicador comparativo â€” **N-Ã©simo** dia da semana vs mÃªs anterior (ex. **3Âª terÃ§a** Ã— **3Âª terÃ§a**) | ğŸ“‹ Pendente Â· foto Word | 16/07 |
| **FL-053** | **P2** | Entrada NF / custo | **Zap #19:** histÃ³rico custo Ãºltimos pedidos **duplica tick** (fim etapa 2 + finalizar NF) | ğŸ“‹ Pendente Â· foto Word | 16/07 |
| **FL-054** | **P1,5** | Entregas / impressÃ£o | **Zap #20:** reimprimir papÃ©is (separaÃ§Ã£o Â· entregador Â· cliente) | ğŸ“‹ Pendente Â· foto Word | 16/07 |
| **FL-055** | **P0,1** | NFC-e / frete | **Zap #23:** rejeiÃ§Ã£o **535** â€” frete no total sem `vFrete` nos itens | âœ… **loja v9.16** | 16/07 |
| **FL-056** | **P0** | NFC-e / SEFAZ | RejeiÃ§Ãµes **963** (fiado+card) + **225** (CFOP/CEST pontuaÃ§Ã£o) â€” vendas #2812/#3347 | ğŸ“¦ **pronto pra envio** Â· teste **v9.21** | 17/07 |
...
| **FL-034** | **P1,9** | PDV / Clientes | BotÃ£o **HistÃ³rico** nÃ£o filtra vendas do **cliente selecionado** â€” deve filtrar (relacionamento / devoluÃ§Ã£o) | ğŸ”„ **F8 modal rascunho** teste Â· fila loja | 29/06 16:20 |
| **FL-035** | **P2** | DevoluÃ§Ã£o | **DevoluÃ§Ã£o parcial** da venda â€” ou **itens especÃ­ficos** | ğŸ“¦ **#12** pronto loja (fecha) Â· âœ… teste Renan | 29/06 16:20 |
| **FL-036** | **P3** | PDV / Promo | **Faixa vertical** ou chaves ligando selos do **mesmo mix** no carrinho (opÃ§Ã£o visual 2) | ğŸ“‹ Pendente | 29/06 |
| **FL-037** | **P3** | PDV / Promo | **Selo mix Ãºnico** entre linhas (rowspan / bloco central â€” opÃ§Ã£o 3 experimental) | ğŸ“‹ Pendente | 29/06 |
| **FL-038** | **P2** | Deploy | **ContingÃªncia deploy** â€” Â§**3.2.0** leigo Â· Â§3.2.4 tÃ©cnico Â· **este deploy = manual Â§3.2** | ğŸ“‹ CÃ³digo pendente | 30/06 |
| **FL-039** | **P3** | Clientes | **Pets/saÃºde/anotaÃ§Ãµes** na **ficha** `/clientes/` (hoje sÃ³ no F8) | ğŸ“‹ Pendente | 30/06 |
| **FL-040** | **P3** | Clientes / PDV | **Tabela Pet** normalizada no Postgres (opÃ§Ã£o B â€” evoluir do JSON) | ğŸ“‹ Pendente | 30/06 |
| **FL-041** | **P3** | PDV | **Fila vendas offline** â€” processar no PC e sync depois (Renan descartou curto prazo) | ğŸ“‹ Pendente | 30/06 |
...
| **FL-048** | **P2** | Ops / Postgres | **Painel backup Postgres** â€” ZIP+Excel+restore Â· `/interno/pg-backup/` Â· Admin | ğŸ§ª **teste** 03/07 | 03/07 |
| **FL-049** | **P1,5** | PDV / Clientes / fiscal | CPF no PDV + NFC-e usa CPF salvo | ? **na loja** (`6af5cac`) | 04/07 |
| **FL-050** | **P2** | Vendas / fiscal | **`/vendas/`** â€” apÃ³s venda, **nÃ£o forÃ§ar** reemissÃ£o ao clicar cupom fiscal enquanto NFC-e roda em **background** Â· avisar *aguarde* Â· rÃ³tulo do botÃ£o muda (emitindo â†’ **reimprimir** quando OK) | ğŸ§ª **teste** 04/07 | 03/07 |
| **FL-042** | **P2** | PDV / Clientes | **HistÃ³rico ERP no F8** â€” **v5.46 teste** Â· import 1Ã— Â· corte ERP **â‰¤26/05** Â· SisVale **â‰¥27/05** | ğŸ§ª Render teste Â· dry-run â†’ import | 30/06 |
| **FL-043** | **P2,8** | Fiado | BotÃ£o **desconto** na **baixa** do fiado | ğŸ“‹ Pendente | 30/06 |
| **FL-044** | **P2,9** | PDV / PreÃ§os / RH | **Desconto automÃ¡tico funcionÃ¡rio** â€” % prÃ©-definida Â· provÃ¡vel junto com **tabelas de preÃ§o Ã— forma de pagamento ou grupo de cliente** (ver **FL-001**) | ğŸ“‹ Pendente | 30/06 |
...
| FL-003 | `pdv-promo-badge-dre-cupom` | **Fase 1 âœ… teste:** selo carrinho `pdv_wizard` + `pdv_promocoes.js` Â· **Fase 2 ğŸ“‹:** DRE/relatÃ³rio Â«desconto clientesÂ» Â· **Fase 3 ğŸ“‹:** cupom 80mm/PDF venda |
...
| Fase | # | O quÃª | Status |
| ---- | - | ----- | ------ |
| **AmanhÃ£** | **10â†’8â†’11** | Backup + congelar + checkpoint CP | **ğŸ“… Renan 30/06** |
...
| **Atalhos (4 slots)** | Clique aplica Â· Shift+clique grava Â· **Alt+clique fixa padrÃ£o** (ğŸ“Œ abre sempre) Â· Postgres global Â· v3.47 |
...
| VariÃ¡vel | Staging |
| -------- | ------- |
| `AGRO_DISPLAY_SCALE_HABILITADO=true` | â• se quiser modal/botÃ£o **Aa** no teste |
...
### 4.16 LanÃ§amentos â€” preparaÃ§Ã£o corte ERP (**feito**) Â· migraÃ§Ã£o Postgres (**ğŸ”¥ HOJE â€” CP**)
...
| **`agro_fonte_config`** | Loja: gate checkpoint ERP sync (import morto â†’ `except`) | ğŸŸ¡ baixo | Teste simplificou (#7 diff); **sem efeito** se env sync off |
...
**OperaÃ§Ã£o:** antes de imprimir lote, abrir ğŸ–¨ï¸ no cadastro â€” se aparecer aviso Ã¢mbar Â«Sem EANÂ», corrigir cadastro primeiro.
...
**Faixa 230â€¦ (interno loja):** gerada pelo SisVale (`agro_codigo_barras_loja_util.py`) â€” 230 + 10 dÃ­gitos sequenciais. **NÃ£o Ã© EAN-13** (Ãºltimo dÃ­gito Ã© sequÃªncia, nÃ£o DV). Etiqueta imprime **CODE128**; modal ğŸ–¨ï¸ mostra Â«Barras interno lojaÂ». EAN real (ex. Abacate `789â€¦`) continua EAN13.
