...
- **PIN LanÃ§amentos:** 1× por sessÃ£o ao entrar em qualquer tela `/lancamentos/*` (sem hub modal); navegaÃ§Ã£o interna sem repetir; **modo descanso** (~3 min idle) pede de novo; sair para PDV/outra tela limpa a sessÃ£o. **Gravar** (Finalizar/baixa/editar): se PIN fresco (~45s) venceu → **teclado** (LANC-PIN-TECLADO v22.91), não alert nativo.
...
| Item | Detalhe |
| ---- | ------- |
| **Escopo** | Import **1×** Mongo `DtoVenda` â†’ Postgres **tabelas separadas** Â· merge sÃ³ no **F8** |
...
| **Resumo** | **9 cards em linha** (1×9): Visitas Â· Ticket Â· Cashback Â· Vale Â· Total Â· Freq. Â· Ãšlt. visita Â· Pets Â· WhatsApp â€” **v5.43** |
...
| ----- | ----------------- | ---- | ------- |
| **A â€” Mongo 1× (recomendado)** | Alta, se gravar em **tabela histÃ³rica separada** | Completo (`DtoVenda` + itens); `ClienteID` casa com `externo_id`; sem planilha manual | Carga Mongo na hora do import; precisa **data corte** + dedup vs PDV |
...
**Recomendado:** **A + B** â€” comando **1×** lÃª Mongo (`DtoVenda` / `DtoVendaProduto`), grava Postgres, **encerra**. Excel sÃ³ para **conferÃªncia**, correÃ§Ã£o ou o que o Mongo nÃ£o casar.
...
| ----- | ------------- | ---- |
| **Mongo ERP (import 1×)** | data venda **â‰¤ 26/05/2026** | HistÃ³rico antigo |
...
| **Teste staging** | Abrir caixa Â· vender Dinheiro Â· devolver Â· **Esperado** deve cair **1×** (nÃ£o 2Ã—) Â· Ctrl+F5 painel caixa |
...
| **FL-042** | **P2** | PDV / Clientes | **HistÃ³rico ERP no F8** â€” **v5.46 teste** Â· import 1× Â· corte ERP **â‰¤26/05** Â· SisVale **â‰¥27/05** | ðŸ§ª Render teste Â· dry-run â†’ import | 30/06 |
...
| FL-042 | `relacionamento-import-erp-1x` | Mongo DtoVenda 1× â†’ Postgres histÃ³rico Â· merge F8 Â· **sem** leitura Mongo no PDV Â· Excel = audit |
...
| **v4.62** | ImpressÃ£o mais leve: JsBarcode 1× na pÃ¡gina, sem pop-up extra no PDV, modais sem blur GPU |
...
| **v3.55** | **`/vendas/`** â€” filtro por data usa Ã­ndice (`criado_em`); JSON pesado adiado; fiado/NFC-e 1× por linha Â· **PDV wizard** â€” catÃ¡logo no **localStorage** (mesma chave da Consulta) + delta em background; nÃ£o refaz download ao fechar Chrome |
...
| **Fix teste** | **v3.53** â€” validade: SQL para lotes com qtd>0 + Mongo sÃ³ Â«conferirÂ» + cache 3 min; vendas: NFC-e config 1× por pÃ¡gina |
