...
| **Novo** | **Despesas por categoria** — fixa/variável/outra · 3 meses ou 3 semanas · tabela + gráfico top 10 · ?% · clique ? CP |
...
### ?? Deploy loja **v6.10** — incompleto (02/07)
...
### ?? FL-017 — confusão teste vs loja (29/06 — histórico)
...
| 4 | Calendário fluxo | **??** vendas incluem vendas **do teste** | **Esperado no staging** — Postgres próprio · na **loja** só vendas loja |
...
| **Modo tempo** | Tempo real · **Como era no dia** · **Comparar** (faixa totais · ? por ponto) |
...
| **Gestão lista** | Loja **Mongo+overlay** · teste **PG v4.36** | ?? loja | Deploy pacote corte quando Renan OK teste |
...
| **Saldo** | GM9503 «teste» **-2** — **Consulta/orçamento** (Centro S) = **Compras** (coluna Saldo) |
...
### ?? Violação protocolo produção — Contabilidade (2026-06-24)
...
**Operação:** antes de imprimir lote, abrir ??? no cadastro — se aparecer aviso âmbar «Sem EAN», corrigir cadastro primeiro.
...
**Faixa 230… (interno loja):** gerada pelo SisVale (`agro_codigo_barras_loja_util.py`) — 230 + 10 dígitos sequenciais. **Não é EAN-13** (último dígito é sequência, não DV). Etiqueta imprime **CODE128**; modal ??? mostra «Barras interno loja». EAN real (ex. Abacate `789…`) continua EAN13.
